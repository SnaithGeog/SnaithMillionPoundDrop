/* =========================
   SNAITH MILLIONAIRE v4.1 (Stable)
   ========================= */
...
// SAFE AUDIO PLAYER - Never crashes the game
async function safePlayAudio(type) {
    if (!el.audio.intro || !el.audio.game) return; // Audio not loaded
    try {
        if(type === 'intro') {
            el.audio.game.pause();
...
if (el.btns.enter) {
    el.btns.enter.addEventListener('click', () => {
        // Force the screen change immediately
        switchScreen('game');
        // Load data
        idx = 0;
        loadQuestion();
        // Try audio last so it doesn't block
        safePlayAudio('game');
    });
}
